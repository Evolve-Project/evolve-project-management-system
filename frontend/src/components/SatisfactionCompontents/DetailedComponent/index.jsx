...
import DetailedShimmer from "../SatisfactionShimmer/detailedShimmer";
...
const SatisfactionDetailed = ({ role, userId, givenByRecords, feedbacks }) => {
  ...
  const userIdMapName = useMemo(
    ...
    [userId, feedbacks, givenByRecords]
  );

  const LENGTH = givenByRecords.length;
  const [selectedIds, setSelectedIds] = useState(
    userIdMapName.map((record) => record.id)
  );
  ...
  if(!feedbacks || !metrics || !bestSort || !worstSort || !userIdMapName || !givenByRecords)
    return <DetailedShimmer/>;
                ...
                renderValue={(selected) => {
                  return selected.length === LENGTH
                    ...
                    : selected
                        .map(
                          (id) =>
                            userIdMapName.find((record) => id === record.id)
                              .name
                        )
                        .join(", ");
                }}
              ...
              >
                ...
                {givenByRecords.map((record) => (
                  <MenuItem key={record.user_id} value={record.user_id}>
                    <Checkbox
                      checked={selectedIds.indexOf(record.user_id) > -1}
                    />
                    <ListItemText
                      primary={
          ...
          return (
            <div className="satisfaction_metric_container" key={metric.id}>
              ...
              {(sortBy === "best"
                ...
                .filter((record) =>
                  selectedIds.includes(record.given_by_user_id)
                )
                .map((feedbackRecord) => {
                  return (
                    <>
                          ...
                          <div>
                            ...
                            <span>{`${
                              givenByRecords.find(
                                (user) =>
                                  user.user_id ===
                                  feedbackRecord.given_by_user_id
                              ).first_name
                            } ${
                              givenByRecords.find(
                                (user) =>
