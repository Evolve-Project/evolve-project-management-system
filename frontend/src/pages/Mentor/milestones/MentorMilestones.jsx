...
function MentorMilestones() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const menteesResponse = await axios.get("http://localhost:8000/api/getMentees");
        console.log("Mentees response:", menteesResponse.data);
        setMentees(menteesResponse.data);

        const milestoneDescResponse = await axios.get("http://localhost:8000/api/get-milestones");
        console.log("Milestones response:", milestoneDescResponse.data);
        setMilestoneDesc(milestoneDescResponse.data);

        const milestoneIdResponse = await axios.get("http://localhost:8000/api/get-milestonesForStatus");
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    try {
      const response = await axios.post("http://localhost:8000/api/update-task-status", { taskId, newStatus });
      console.log("Update status response:", response.data);

      // Update the status of the task in the frontend
      setTasks(tasks.map((task) => (task.id === taskId ? { ...task, status: newStatus === "true" } : task)));
    } catch (error) {
  ...
  const deleteTask = async (taskId) => {
    try {
      const response = await axios.post("http://localhost:8000/api/delete-task", { taskId });
    ...
    <>
      {toggle ? (
        ...
        <>
            ...
            <tbody className="text-black-600 text-bg font-dark">
              {currentTasks.map((task, index) => (
                <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                  ...
                  <td className="py-2 px-3 text-left">{getMenteeName(task.mentee_user_id)}</td>
                  <td className="py-2 px-3 text-left">
                    <button className="text-red-600 hover:text-red-900 focus:outline-none" onClick={() => deleteTask(task.id)}>
                      <svg
                        ...
                        stroke="currentColor"
                      >
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 6l3 15h12l3-15H3zm9 2v8m-4-4h8" />
                      </svg>
                    </button>
                  </td>
                </tr>
          ...
          <button className="fixed top-3 right-5 mt-2 mr-2 bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-gray-600" onClick={handleBackButtonClick}>
            Back
          </button>
        </>
      )}
